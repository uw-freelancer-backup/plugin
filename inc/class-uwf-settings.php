...
class UW_Freelancer_Setttings{    
   ...
   private $api_console_hook_suffix;
   ...
   function register_menus() {
       ...
       $this->freelancer_hook_suffix = add_menu_page(
               ...
               array($this, 'freelancer_dashboard'),
               UWF_URL .'images/uw-freelancer-icon.png');
       
       add_action("load-{$this->freelancer_hook_suffix}",array($this,'freelancer_help_screen'));
       add_filter("uwf_admin_pointers-{$this->freelancer_hook_suffix}", array($this, 'freelancer_pointers' ));

       $this->api_console_hook_suffix = add_submenu_page(
               'uw-freelancer-settings', 
               __('API Console', 'uwf'), 
               __('API Console', 'uwf'), 
               'manage_options', 
       ...
       add_action("load-{$this->api_console_hook_suffix}",array($this,'api_console_help_screen'));
       add_filter("uwf_admin_pointers-{$this->api_console_hook_suffix}", array($this, 'api_console_pointers' ));
       
       $this->transients_hook_suffix = add_submenu_page(
               'uw-freelancer-settings', 
               __('Transients', 'uwf'), 
               __('Transients', 'uwf'), 
               'manage_options', 
               'uw-freelancer-transients', 
               array($this, 'freelancer_transients'));
       
       add_action("load-{$this->transients_hook_suffix}",array($this,'transients_help_screen'));
       add_filter("uwf_admin_pointers-{$this->transients_hook_suffix}", array($this, 'transients_pointers' ));       
   ...
   function get_page_tabs_dashboard() {
      $tabs = array(
      'profile' => __('Profile Widget', 'uwf'),
      'feedback' => __('Feedback Widget', 'uwf'),
      'affiliate' => __('Affiliate Widget', 'uwf'),
      'styling' => __('Widget Styling' , 'uwf')
      );
      return $tabs;
   }    
   
   function get_page_tabs_transients() {
      $tabs = array(
      'introduction' => __('Introduction', 'uwf'),
      'user' => __('User Transient', 'uwf'),
      'feedback' => __('Feedback Transient', 'uwf')
      );
      return $tabs;
   }    

   function freelancer_dashboard() { 
       echo '<div class="wrap uwf-background">';
           $this->page_tabs_dashboard(); 
           settings_errors();
           ...
           if ( 'admin.php' == $pagenow && isset( $_GET['page'] ) && 'uw-freelancer-settings' == $_GET['page'] ) :
                ...
                switch ( $tab ) :
                     ...
                     case 'affiliate' :
                          ...
                          break;
                     case 'styling' :
                          require( plugin_dir_path(__FILE__) . '/templates/options-register-styling.php' );
                          break;                       
                endswitch;
           endif;                
   ...
   function freelancer_transients(){
       ...
       echo '<div class="wrap uwf-background">';
           $this->page_tabs_transients(); 
           settings_errors();
       echo '<form action="options.php" method="post">';
           global $pagenow;
           if ( 'admin.php' == $pagenow && isset( $_GET['page'] ) && 'uw-freelancer-transients' == $_GET['page'] ) :
                if ( isset ( $_GET['tab'] ) ) :
                     $tab = $_GET['tab'];
                else:
                     $tab = 'introduction';
   ...
   function freelancer_api_console(){
       ...
       echo '<div class="wrap uwf-background">';           
           ...
           echo '<div id="uwf-icon" class="icon32"><br /></div>';
   ...
   function page_tabs_dashboard( $current = 'profile' ) {
        if ( isset ( $_GET['tab'] ) ) :
             $current = $_GET['tab'];
        else:
             $current = 'profile';
        endif;
        $tabs = $this->get_page_tabs_dashboard();
        $links = array();
        ...
        endforeach;
        echo '<div id="uwf-icon" class="icon32"><br /></div>';
        echo '<h2>' . __('Freelancer Dashboard', 'uwf') . '</h2>';
   ...
   function page_tabs_transients( $current = 'introduction' ) {
        if ( isset ( $_GET['tab'] ) ) :
             $current = $_GET['tab'];
        else:
             $current = 'introduction';
        endif;
        $tabs = $this->get_page_tabs_transients();
        $links = array();
        foreach( $tabs as $tab => $name ) :
             if ( $tab == $current ) :
                  $links[] = "<a class='nav-tab nav-tab-active' href=?page=uw-freelancer-transients&tab=" . $tab . ">$name</a>";
             else :
                  $links[] = "<a class='nav-tab' href=?page=uw-freelancer-transients&tab=" . $tab . ">$name</a>";
             endif;
        endforeach;
        echo '<div id="uwf-icon" class="icon32"><br /></div>';
        echo '<h2>' . __('Freelancer Transients', 'uwf') . '</h2>';
        echo '<h2 class="nav-tab-wrapper">';
        foreach ( $links as $link )
             echo $link;
        echo '</h2>';
   ...
   if ( $submit_profile ) {
       ...
       $valid_input['transient_timeout'] = $default_options['transient_timeout']; 
       $valid_input['clear_transients'] = $default_options['clear_transients'];
   ...
   function freelancer_admin_scripts(){      
       ...
       if(isset( $_GET['page'] ) 
          && ( 'uw-freelancer-settings' == $_GET['page']
                ...
                || 'uw-freelancer-advanced-settings' == $_GET['page'] 
                || 'uw-freelancer-transients' == $_GET['page']  ))
       {
   ...
   function freelancer_help_screen(){
      require UWF_PATH . 'inc/help/help-main.php';      
   }
   
   function freelancer_pointers($p){
      require UWF_PATH . 'inc/help/pointer-main.php';
      return pointer_main($p);
   }    
   
   function api_console_help_screen(){
      require UWF_PATH . 'inc/help/help-console.php';      
   }   
   
   function api_console_pointers($p){
      require UWF_PATH . 'inc/help/pointer-console.php';
      return pointer_console($p);
   }   
   
   function transients_help_screen(){
      require UWF_PATH . 'inc/help/help-transients.php';      
   }   
   
   function transients_pointers($p){
      require UWF_PATH . 'inc/help/pointer-transients.php';
      return pointer_transients($p);
   }     
}
?>